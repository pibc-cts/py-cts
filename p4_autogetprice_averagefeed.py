...
if bqg[-1] > sqg[-1] :
	...
	for i in range(0, len(sqg)):
		if sqg[i]>=bqg[-1]:
			...
			#print("相同",bqg[-1],"数量下,","买单总价:",bb[-1],"卖单总价:",sum_ask_price)
...
print("maket_price:",cny_math_order)
cny_feed =round((cny_settlement_price+cny_math_order)/2,5)
print("average:",cny_feed)
#feed_choice=input("请判断是否使用上方数值进行喂价.y/n\n")
#if feed_choice == 'y':
publish_feedprice(account_name, cny_feed, wallet_port)
#else:
exit(0)
